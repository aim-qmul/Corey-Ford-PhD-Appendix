...
# Label size for consistency
label_size <- 15

# Reshape to long format
sris_subset_long <- pivot_longer(
  sris_subset,
  cols = everything(),
  names_to = "variable",
  values_to = "value"
...
means <- sris_subset_long %>%
  ...
  summarise(
    ...
    ymin = mean_value - t_star * se,
    ymax = mean_value + t_star * se
  )

# Build the plot
sris_plot <- ggplot(sris_subset_long, aes(x = variable, y = value, fill = variable)) +
  
  # Boxplot
  geom_boxplot(aes(colour = variable), fill = "transparent", outlier.shape = 1, width = 0.3) +
  
  # CI error bars nudged right
  geom_errorbar(data = means,
                aes(x = variable, ymin = ymin, ymax = ymax),
                position = position_nudge(x = 0.3),
                width = 0.2, colour = "#656565", inherit.aes = FALSE) +
  
  # Mean point over error bars (grey, nudged right)
  geom_point(data = means,
             aes(x = variable, y = mean_value),
             position = position_nudge(x = 0.3),
             shape = 4, fill = "white", colour = "#656565", size = 3, stroke = 0.5) +
  
  # Mean point centered on boxplot (colored by variable)
  geom_point(data = means,
             aes(x = variable, y = mean_value, colour = variable),
             shape = 4, size = 3) +
  
  # Y-axis limits and breaks
  scale_y_continuous(breaks = seq(1, 6, by = 1.0), limits = c(1, 6)) +
...
# Subscale box plot
msi_av_plot <- ggplot(msi_subset_long, aes(x = variable, y = value, fill = variable)) +
  geom_boxplot(aes(colour = variable), fill = "transparent", outlier.shape = 1, width = 0.3) +
  
  geom_errorbar(data = msi_means,
                aes(x = variable, ymin = ymin, ymax = ymax),
                position = position_nudge(x = 0.3),
                width = 0.2, colour = "#656565", inherit.aes = FALSE) +
  
  geom_point(data = msi_means,
             aes(x = variable, y = mean_value),
             position = position_nudge(x = 0.3),
             shape = 4, fill = "white", colour = "#656565", size = 3, stroke = 0.5) +
  ...
  ylab("Goldsmiths MSI - Subscale Scores\n") +
  xlab("")
...
# Reshape to long format
gmssum_subset_long <- pivot_longer(
  gmssum_subset,
  cols = everything(),
  names_to = "variable",
  values_to = "value"
...
# GMS total score box plot
gmsum_plot <- ggplot(gmssum_subset_long, aes(x = variable, y = value)) +
  geom_boxplot(fill = "transparent", colour = "navy", outlier.shape = 1, width = 0.3) +
  
  geom_errorbar(data = gms_means,
                aes(x = variable, ymin = ymin, ymax = ymax),
                position = position_nudge(x = 0.3),
                width = 0.2, colour = "#656565", inherit.aes = FALSE) +
  
  geom_point(data = gms_means,
             aes(x = variable, y = mean_value),
             position = position_nudge(x = 0.3),
             shape = 4, fill = "white", colour = "#656565", size = 3, stroke = 0.5) +
  ...
  geom_point(data = gms_means,
             aes(x = variable, y = mean_value),
             shape = 4, colour = "navy", size = 3) +
  
  scale_y_continuous(breaks = seq(18, 126, by = 12), limits = c(18, 126)) +
  scale_x_discrete(labels = c(
    "MSI_GMS_sum" = "MSI\nTotal Score\n(Mean=67.7)\n(Med=63.5)\n(SD=21.5)"
  )) +
  theme_minimal() +
  ...
  ylab("Goldsmiths MSI - Total Score\n") +
  xlab("")


# ─── Arrange Both Plots Side by Side ──────────────────────────────

grid.arrange(gmsum_plot, msi_av_plot, nrow = 1, widths = c(0.3, 0.7))
...
ueq_subset_long <- pivot_longer(ueq_subset,
                                cols = everything(),
                                names_to = "variable",
                                values_to = "value")
...
# Plot
ueq_plot <- ggplot(ueq_subset_long, aes(x = variable, y = value, fill = variable)) +
  geom_boxplot(aes(colour = variable), fill = "transparent", outlier.shape = 1, width = 0.3) +
  geom_errorbar(data = gms_means,
                aes(x = variable, ymin = ymin, ymax = ymax),
                position = position_nudge(x = 0.3),
...
# Reshape to long format using pivot_longer
rice_subset_long <- pivot_longer(rice_subset,
                                 cols = everything(),
                                 names_to = "variable",
                                 values_to = "value")
...
# Plot
rice_plot <- ggplot(rice_subset_long, aes(x = variable, y = value, fill = variable)) +
  geom_boxplot(aes(colour = variable), fill = "transparent", outlier.shape = 1, width = 0.3) +
  geom_errorbar(data = rice_means,
                aes(x = variable, ymin = ymin, ymax = ymax),
                position = position_nudge(x = 0.3),
                width = 0.2, colour = "#656565", inherit.aes = FALSE) +
  geom_point(data = rice_means,
             aes(x = variable, y = mean_value),
             position = position_nudge(x = 0.3),
             shape = 4, fill = "white", colour = "#656565", size = 3, stroke = 0.5) +
  geom_point(data = rice_means,
...
# Statistics for RiCEv2 Analysis ===============================================
# Load data 
...
equation_text <- "RiCEv2=-3.99+(1.11)Focused_Attention\n              +(1.41)Reward*"  # Replace with your actual equation

# Create the ggplot
p <- ggplot(train_data) +
  geom_point(aes(x = UEQ_Focused_Attention, y = RiCE, 
                 color = "UEQ_Focused_Attention", 
                 shape = "UEQ_Focused_Attention"), 
             size = 5, stroke = 0.8, fill = "NA") +
  
  geom_point(aes(x = UEQ_Reward, y = RiCE, 
                 color = "UEQ_Reward", 
                 shape = "UEQ_Reward"), 
             size = 5, stroke = 0.8, fill = "NA") +
  
  geom_smooth(data = trend_data, 
              aes(x = (UEQ_Focused_Attention + UEQ_Reward) / 2, 
                  y = predicted_RiCE, 
                  color = "Model Fit"), 
              method = "lm", se = FALSE, color = "black") +
  
  annotate("text", x = 1.4 , y = 0, label = equation_text, 
           hjust = 0, color = "black", angle = 44, size = 5) +
...
p <- ggplot(train_data) +
  ...
  annotate("text", x = 1.15, y = 0, label = equation_text, hjust = 0, color = "black", angle = 44, size = 5) +
  scale_x_continuous(limits = c(1, 5), breaks = seq(1, 5, by = 1)) +
  scale_y_continuous(limits = c(0, 10), breaks = seq(0, 10, by = 2)) +
...
p <- ggplot(train_data) +
  ...
  annotate("text", x = 1.5, y = 0.4, label = equation_text, hjust = 0, color = "black", angle = 31, size = 5) +
  scale_x_continuous(limits = c(1, 5), breaks = seq(1, 5, by = 1)) +
  scale_y_continuous(limits = c(0, 10), breaks = seq(0, 10, by = 2)) +
  ...
  labs(color = NULL, shape = NULL) +
  guides(shape = guide_legend(override.aes = list(linetype = 0), nrow = 2, ncol = 2))
print(p)
...
found_model <- lm(formula = `RiCE-Se` ~ UEQ_Reward, data = train_data)
summary(found_model)
...
p <- ggplot(train_data) +
  ...
  annotate("text", x = 2.6, y = 0, label = equation_text, hjust = 0, color = "black", angle = 45, size = 5) +
  scale_x_continuous(limits = c(1, 5), breaks = seq(1, 5, by = 1)) +
  scale_y_continuous(limits = c(0, 10), breaks = seq(0, 10, by = 2)) +
